...
export default function ChangeDiseases({
  patientDiseases,
  diseases,
  name,
  GoBack,
  ...
  function onCreate() {
    ...
    navigate("/pattern/new", {
      state: { itemDisease: diseases, pattern: disease, name: nam },
    });
  }
  ...
  return (
    <>
      {patientDiseases.map((d, index) => (
        <Row key={d.id} className="mb-3 text-center">
          <Col>{index ? "" : <Button onClick={GoBack}>GoBack</Button>}</Col>
          <Col>
            {index ? "" : <p onClick={() => setDel(0)}>{translation[name]}</p>}
          </Col>

          <Col>
            <p onClick={() => onPClick(d.id)}>{d.name}</p>
          </Col>
          <Col>
            {del === d.id ? (
              <Button onClick={() => onDelete(d)}>Удалить</Button>
            ) : (
              ""
            )}
          </Col>
          <Col>
            {index || del ? "" : <Button onClick={onEdit}>Сохранить</Button>}
          </Col>
        </Row>
      ))}
